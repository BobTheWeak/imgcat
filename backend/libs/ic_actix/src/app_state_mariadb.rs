...
pub type AppStateMariaDB = Data<ICMariaDBWrapper>;

pub struct ICMariaDBWrapper {
	pool: Pool // Thread-safe, so we don't need a mutex
}

impl ICMariaDBWrapper {
	pub fn new(host:&str, port:u16, db:&str, user:&str, pass:&str) -> ICResult<Self> {
		...
		let cfg = OptsBuilder::new()
			...
			// defaults
			.stmt_cache_size(Some(0)) // Don't prepare any statements
			.prefer_socket(false) // The server isn't local, don't try

			// Pool-specific options
			.pool_opts(Some(
				PoolOpts::new()
				// NOTE: The default setting is 10-100 connections, which seems insane
				.with_constraints(PoolConstraints::new(1, 25).unwrap())
		...
		if client.query_drop("SELECT 1").is_err() {
			return Err(ICError::MARIADB_CONN);
		}
	...
	pub fn new_with_defaults(db:&str, user:&str, pass:&str) -> ICResult<Self> {
