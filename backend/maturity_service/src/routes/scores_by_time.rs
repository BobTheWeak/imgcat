...
#[derive(Deserialize)]
pub struct BulkFromParams {
	#[serde(rename="t")]
	last_timestamp: i64,
}


pub async fn scores_by_time(
		params: web::Query<BulkFromParams>,
	...
	let result = match conn.query_typed(
		"SELECT * FROM Results.GetMaturityScoresByTime($1)",
		&[ (&params.last_timestamp, Type::INT8) ]
	).await {
		Ok(data) => {
