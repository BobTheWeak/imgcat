...
use crate::redis_helpers::{get_post_votes, set_post_votes};
...
pub async fn get_votes(
	...
	if let Some(votes) = get_post_votes(&mut r_conn, &path.post_link)? {
	...
	let _ = set_post_votes(&mut r_conn, &path.post_link, votes);
