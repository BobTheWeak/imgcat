...
use crate::db_objects::{ ContentLevel, VisibilityLevel, ContentWeight };
...
pub async fn set_account_preferences(client:&Client, prefs:&AccountPreferencesSetter) -> ICResult<i32> {
	...
	let mut idx:u32 = 1;
	let mut query:String = "SELECT UsersSvc.SetAccountPreferences(account_id:=$1".to_string();
